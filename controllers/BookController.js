...
class BookController{
    ...
    static addPost(req, res){
        ...
        Book.create(newBook)
            .then(data =>{
                res.redirect('/books/admin?message=berhasil menambahkan buku')
            })
            .catch(err =>{
                let message = []
                if(Array.isArray(err.errors)){
                    ...
                    res.redirect(`/books/add?message="${message.join(',')}"`)
                }
                else{
                    res.render('error', {error:err})
                }  
    ...
    static updatePost(req, res){
        ...
        Book.findByPk(getId)
            ...
            .then(data =>{
                res.redirect('/books/admin?message=berhasil mengupdate buku')
            })
            .catch(err =>{
                let message = []
                if(Array.isArray(err.errors)){
                    ...
                    res.redirect(`/books/update/${getId}?message="${message.join(',')}"`)
                }
                else{
                    res.render('error', {error:err})
                }  
    ...
    static delete(req, res){
        ...
        Book.destroy({where:{
            ...
            returning: true})
        .then(data =>{
            res.redirect('/books/empty?message=berhasil menghapus buku')
        })
    }
